-- Creation de l'extension pgcrypto pour pouvoir generer des UUID automatiquement
...
-- =========================================
-- TABLE : jetons_revoques (Securite / deconnexion)
-- =========================================
...
-- =========================================
-- TABLE : categories (Categories de depenses)
-- =========================================
-- Ces categories sont alignees avec celles predites par l'IA.
CREATE TABLE categories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    nom VARCHAR(100) UNIQUE NOT NULL,
    code_couleur_hex VARCHAR(7) DEFAULT '#000000', -- Couleur pour l'affichage
    icone VARCHAR(50), -- Emoji ou identifiant d'icone
    cree_le TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
('Divers/Bazar', '#f59e0b', '🧺'),
('Entretien', '#64748b', '🧽'),
('Hygiène et Beauté', '#ec4899', '🧴'),
('Restauration', '#a855f7', '🍽️');

-- =========================================
-- TABLE : tickets (Factures et recus scannes)
-- =========================================
-- Represente un ticket scanne par l'IA ou saisi manuellement.
CREATE TABLE tickets (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    utilisateur_id UUID NOT NULL REFERENCES utilisateurs(id) ON DELETE CASCADE,

    -- Donnees extraites par l'IA / OCR
    nom_marchand VARCHAR(255) NOT NULL,
    montant_total NUMERIC(10, 2) NOT NULL,
    date_achat DATE NOT NULL,

    -- Categorie globale du ticket
    categorie_id UUID REFERENCES categories(id) ON DELETE SET NULL,
    est_exceptionnel BOOLEAN NOT NULL DEFAULT FALSE,

    -- Statut du traitement ("valide", "en attente", "erreur")
    statut VARCHAR(50) DEFAULT 'en attente',

    -- Metadonnees de l'IA
    confiance_ocr NUMERIC(5, 2), -- Score de confiance de l'IA (0 a 100)
    url_image TEXT, -- Lien vers l'image stockee
    texte_brut_extrait TEXT, -- Texte brut retourne par l'OCR
...
-- =========================================
-- Si l'IA extrait ligne par ligne, on stocke ici le detail des achats.
...
-- =========================================
-- TRIGGERS pour la mise a jour automatique
